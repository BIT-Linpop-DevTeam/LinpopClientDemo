...
QString EmojiModel::emojiChar(QString category, int index) {
    QJsonObject emoji = emojies[category][index].toObject();

        return emoji.value("char").toString();
}
...
QVector<QString> EmojiModel::searchEmojiChar(QString searchKey) {
    bool foundFirstItem{false};
    QVector<QString> searchResultEmojiChar;
    for (auto it{keywords.begin()}; it != keywords.end(); ++it) {
        if (it.key().startsWith(searchKey)) {
            QVector<QJsonObject>& emojiesData{it.value()};
            for (auto emoji{emojiesData.begin()}; emoji != emojiesData.end(); ++emoji) {
                    QString emojiChar = emoji->value("char").toString();
                    if (!searchResultEmojiChar.contains(emojiChar))
                        searchResultEmojiChar.append(emojiChar);
            }
            foundFirstItem = true;
        }
        else if (foundFirstItem) {
