...
Item {
    ...
    property var searchModelEmojiChar: ListModel {}
    ...
    function refreshSearchModel() {
        searchModel.clear()
        searchModelEmojiChar.clear()
        var searchResult = model.search(searchField.text, skinColor)
        var searchResultEmojiChar = model.searchEmojiChar(searchField.text)
        for (var i = 0; i < searchResult.length; ++i) {
            searchModel.append({path: searchResult[i]})
            searchModelEmojiChar.append({emojiChar: searchResultEmojiChar[i]})
        }
    }
    ColumnLayout {
        anchors.fill: parent
        RowLayout {
            ...
            TextField {
                ...
                onTextChanged: {
                    ...
                    text.length > 0 ? container.refreshSearchModel() : container.searchModelEmojiChar.clear()
                }
            }
            Repeater {
                id: cateIcons
                ...
                Repeater {
                    ...
                    delegate: Rectangle  {
                        ...
                        MouseArea {
                            ...
                            cursorShape: Qt.PointingHandCursor
                            onClicked: {
                                qmlSignal(container.searchMode ? container.searchModelEmojiChar.get(index).emojiChar: container.model.emojiChar(grid.category, index));
                            }
                        }
                    }
                }
